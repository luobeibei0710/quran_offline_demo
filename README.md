...
> 9 项验收矩阵中 **3 项满足、2 项部分满足、4 项未满足**。
> **在矩阵补齐之前，广播链路不得声称已验收。**

| 维度 | 目标 | 状态 |
|------|------|------|
| 连续收听时长 | ≥ 30 分钟不间断 | ✅ 33 分钟，0 崩溃（内存未采样） |
| 定位正确性 | 节号随音频严格单调递增 | ✅ 满足（2 处表面跳跃已逐条查证：音源跳章、著名相似节的真实歧义） |
| 译文来源 | `curatedEdition` 占比接近 100% | ✅ 95.2%（40/42；另 2 条 `machineAsr` 落在未匹配上，属设计内兜底） |
| 章节覆盖 | 长章 / 短章 / 重复节章 / 极短节密集章 | ⚠️ 部分满足，缺短章（纯洁章）与重复节章（至仁主章） |
| 端到端延迟 P95 | 需要有明确目标阈值 | ⚠️ 已测得 P95 5733 ms，但阈值未定，无法判通过与否 |
| 诵读者覆盖 | ≥ 3 位不同诵读者 | ❌ 仅 1 位（Alafasy） |
| 库外负样本 | 求护词 / 解说 / 非经文 → 必须未匹配 | ❌ 未覆盖（2 条 `unmatched` 经查内容仍是经文） |
| 拾音条件 | 距离 / 音量 / 底噪至少两档对照 | ❌ 仅 1 档，距离与音量未记录 |
| iOS 真机 | 本条链路在 iOS 上跑通 | ❌ 未做 |
...
- **广播链路的真机验收远未完成（最重要的一条）**。首轮 33 分钟全经连续播放已证明链路在
  真实外放拾音下可用（42 条记录、0 崩溃、节号单调递增、`curatedEdition` 占 95.2%），
  但 9 项验收矩阵只有 3 项满足，**多诵读者、库外负样本、拾音分档、iOS 真机四项完全未做**，
