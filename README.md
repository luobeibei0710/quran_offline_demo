...
| 结果 | 即时显示 | 与语料原文的逐词比对（覆盖率 / 准确率 / F1）+ 章节命中 |
...
1. 主界面右上角「语料验证」→ 语料列表（**内置多节连续诵读** + 设备上的自定义语料）；
2. 点一条即开始灌音：20 ms 分块、按实时节奏喂入流式会话（含匹配、进度、窗口推进），
   列表上显示进度与该条的关键事件；
3. 跑完**自动进入「原文 / 转写」比对页**：原文 = 该语料的经文库标准经文（内置语料）或你提供的
   原文（自定义语料），转写 = 本轮转写稿；返回列表后该条保留 `章节 n/m` 与 F1；
4. 右上角「全部跑一遍」批量灌音并汇总，日志打印
   `语料验证完成：章节命中 n/m 节；整段全中 k/j 条`。

### 语料来源

| 来源 | 位置 | 说明 |
|------|------|------|
| 内置语料 | `assets/quran_offline/corpus/`（由 `manifest.json` 描述） | `bash tools/quran_offline/download_corpus.sh` 从 Quran.com CDN 取逐节诵读、拼接成 16 kHz 单声道 WAV；**音频与模型一样不入版本库**，clone 后需先跑该脚本 |
...
| `36:1-5` | 5 | 16 | 28 s |
| `55:1-13` | 13 | 47 | 87 s |
| `67:1-11` | 11 | 126 | 159 s |

### 转写稿口径（关键）

转写稿 = **稳定命中章节的标准经文**（按识别顺序、按「新覆盖到的节」累加，同节只记一次），
不是逐词 ASR 输出。原因：流式窗口会反复覆盖同一段音频（159 s 语料被切成 209 轮），逐词输出的
重叠去重必然失效、累积重复内容（实测 47 词的语料拼出 150 词、126 词的语料拼出 1258 词），
无法作为长音频的转写。逐词原始输出仍打在日志里（`逐词原始输出 N 词`）供诊断。

因此页面上的指标含义是：

- **覆盖率 = 原文的词有多少被识别到**（漏念/漏识别体现为缺失词）——这是「准确率」的主指标；
- **F1 偏低通常来自「多余词」**：引擎在短窗口上也会稳定地误认成别的短句（如 `2:1`「الم」、
  `1:3`「الرحمن الرحيم」），这些词会进转写稿，拉低准确率；
- **章节命中 n/m**：语料区间里的节被识别到多少（`整段全中`指整条语料一节不漏）。

### 真机实测（Redmi 24117RK2CC / Android 16 / arm64）

| 语料 | 原文 | 转写稿 | 一致词 | 覆盖率 | F1 | 结论 |
|------|------|--------|--------|--------|-----|------|
| `36:1-5` | 16 词 | 27 词 | 16 | **1.000** | 0.744 | 一般 |
| `55:1-13` | 47 词 | 54 词 | 43 | 0.926 | **0.861** | 良好 |
| `67:1-11` | 126 词 | 209 词 | 126 | **1.000** | 0.752 | 良好 |

**汇总：章节命中 25/29 节，整段全中 2/3 条**。残留误差主要来自「短句误匹配」（见「已知限制」），
不是抓音问题，也不是指标口径问题。
