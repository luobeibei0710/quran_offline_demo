...
class BroadcastTranscriber {
  ...
  /// 对有界预览窗口做一次前向推理，解码与匹配复用同一份声学证据。
  ///
  /// 这里只供候选预览使用。终稿仍调用 [transcribe] 的声学停顿分段路径，
  /// 避免短窗近似结果改变历史记录的准确度口径。
  Future<BroadcastFragment> transcribePreview(
    Float32List samples, {
    required int offsetSample,
  }) async {
    if (samples.isEmpty) {
    ...
    final transcript = TimedTranscript()
      ..update(
        ...
        windowStart: offsetSample / sampleRate,
        windowEnd: (offsetSample + samples.length) / sampleRate,
        isFinal: true,
      );
    return BroadcastFragment(
      ...
      segments: <BroadcastSegment>[
        BroadcastSegment(
          startSample: offsetSample,
          endSample: offsetSample + samples.length,
          evidence: evidence,
          decoded: decoded,
  ...
  Future<BroadcastFragment> transcribe(
    ...
    for (var index = 0; index < bounds.length; index++) {
      ...
      final evidence = await runner.run(
        Float32List.sublistView(samples, bound.$1, bound.$2),
      );
      final decoded = decoder.decode(
        evidence.logprobs,
        evidence.timeSteps,
      ...
      final isFinal =
          index == bounds.length - 1 || bounds[index + 1].$1 >= bound.$2;
      transcript.update(
        decoded: decoded,
        vocab: vocab,
        frames: evidence.timeSteps,
      ...
      segments.add(
        BroadcastSegment(
          ...
          decoded: decoded,
          forcedBoundary:
              bound.$2 - bound.$1 >= (windowSeconds * sampleRate).round() - 1,
        ),
      );
    ...
    final AcousticEvidence? matchEvidence = segments.isEmpty
        ? null
        : (segments.length == 1
              ? segments.first.evidence
              : await runner.run(samples));
    return BroadcastFragment(
      words: List<String>.unmodifiable(transcript.words),
      timedWords: List<TimedWord>.unmodifiable(transcript.timedWords),
