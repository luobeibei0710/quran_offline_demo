...
class QuranVerse {
  ...
  String get normalizedText =>
      QuranText.normalize(textClean.isEmpty ? textUthmani : textClean);
...
class QuranAssets implements VerseIndex {
  ...
  static Future<QuranAssets> loadVocabularyOnly({AssetBundle? bundle}) async {
    ...
    final vocab = await _loadVocab(assets, vocabAssetKey);
  ...
  /// The vocabulary bundled with the SDK. The legacy demo keeps its own assets.
  static const String vocabAssetKey =
      'packages/quran_broadcast_sdk/assets/quran_offline/vocab.json';
  ...
  static Future<QuranAssets> load({AssetBundle? bundle}) async {
    ...
    final vocab = await _loadVocab(assets, '$assetDir/vocab.json');
    final blankId = _blankIdOf(vocab);

    final quranRaw =
        jsonDecode(await assets.loadString('$assetDir/quran.json'))
            as List<dynamic>;
    ...
    final spanRaw =
        jsonDecode(await assets.loadString('$assetDir/quran_ctc_tokens.json'))
            as Map<String, dynamic>;
    final spanTokens = <String, List<int>>{};
    for (final entry in spanRaw.entries) {
      final list = entry.value as List<dynamic>;
      spanTokens[entry.key] = list
  ...
  static Future<Map<int, String>> _loadVocab(
    AssetBundle assets,
    String key,
  ) async {
    final raw =
        jsonDecode(await assets.loadString(key)) as Map<String, dynamic>;
