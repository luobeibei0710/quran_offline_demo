...
void main() {
  ...
  Future<CorpusRunner> buildRunner({String? expectedRef, Float32List? samples}) async {
  ...
  group('CorpusRunner.run', () {
    test('灌音跑完：产生事件、拼出转写、命中期望章节', () async {
      final runner = await buildRunner(expectedRef: '1:1');
    ...
    test('期望章节与识别结果不符时判为未命中', () async {
      final runner = await buildRunner(expectedRef: '112:1');
    ...
    test('自定义语料（无期望章节）不做命中判定', () async {
      ...
      expect(result.expectedRef, isNull);
    ...
    test('取消后不再灌音，也不产生事件', () async {
      final runner = await buildRunner(expectedRef: '1:1')..cancel();
