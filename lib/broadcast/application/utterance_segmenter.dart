...
class UtteranceSegmenter {
  ...
  void addChunk(Float32List chunk) {
    ...
    _silenceSeconds = _isQuiet(rms) ? _silenceSeconds + chunk.length / sampleRate : 0;
  }

  /// 片段是否已到达自然停顿终点。
  bool get shouldCloseOnSilence =>
      _silenceSeconds >= config.silenceSeconds && _pending.length / sampleRate >= config.minSpeechSeconds;

  /// 片段是否已超过最长时长。
  bool get shouldForceSplit => _pending.length / sampleRate >= config.maxSeconds;
  ...
  SpeechSegment _take(SegmentBoundary reason, {required bool keepOverlap}) {
    ...
    if (keepOverlap) {
      ...
      final keep = Float32List.sublistView(_pending, _pending.length - overlapSamples);
  ...
  bool _hasSpeech() => _pendingPeak >= math.max(config.minRms * config.speechRmsFactor, _baseline * config.speechRmsFactor);

  bool _isQuiet(double rms) => rms < math.max(config.minRms, _baseline * config.silenceRmsFactor);
